...
public class BusTicketing {
	...
	public void busTicketing() throws IOException {
		...
		for (int i = 1; i < 4;) {
			...
			if (user.equals("333") && password.equals("333")) {
				...
				for (int x = 1; x == 1;) {
					...
					System.out.println(" 2. 버스 티켓 예매 ");
					System.out.println(" 3. 버스 티켓 계산");
					System.out.println(" 4. 탑승객 티켓 구매 조회");
					System.out.println(" 5. 탑승객 티켓 구매 변경");
					...
					for (x = 1; x == 1;) {
						...
						if (choice.equals("1")) {
							...
							if ((available[1] == 0) && (available[2] == 0) && (available[3] == 0) && (available[4] == 0)
								...
								for (x = 1; x == 1;) {
									...
									for (int l = 0; l < z; l++) {
										if (ticketS[l][0].equalsIgnoreCase(ticketS[z][0])) {
											System.out.println("이미 구매한 탑승자입니다.");
											x = 1;
										}
									}
								}
								...
								for (x = 1; x == 1;) {
									...
									for (int p = 1; p <= 5; p++) {
										if (to == p) {
											...
											if (available[to] < 0) {
												System.out.print("죄송합니다 현재 " + ticketI[z][0] + " 명 분의 좌석이 없습니다.");
												available[to] = available[to] + ticketI[z][0];
												System.out.print("우리는 오직 " + available[to] + " 명 분의 좌석이 예약이 가능합니다.");
												x = 1;
												print = 0;
											} else {
												x = 0;
								...
								for (x = 1; x == 1;) {
									System.out.print("할인받을 수 있는 사람 명수 입력: ");
